...
describe('Strategy6BatchEvaluation', () => {
  ...
  it('filters EMA confirmation independently from score and supports multiple grades', async () => {
    ...
    const grades = wrapper.get('[data-test="ema-grade-options"]')
    expect(grades.classes()).toContain('grade-options')
    expect(grades.findAll('label')).toHaveLength(6)
    expect(grades.text()).toContain('S · 极致缠绕（90–100分）')
    expect(grades.text()).toContain('未达标（低于50分）')
