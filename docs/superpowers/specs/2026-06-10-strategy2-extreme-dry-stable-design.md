...
9. 数据源按现有四数据源机制运行，策略2不改变数据源优先级、标准化格式和入库方式。
...
```text
共享层
股票池 → 四数据源 → 日线标准化/入库 → 全局流动性过滤 → 全局扫描互斥
                                                   ├─ 策略1现有链路
                                                   └─ 策略2独立链路
```
...
该模块只包含数据源链、互斥、重试、缓存新鲜度和统一 `FetchResult`，不得包含任何策略判断。策略1与策略2扫描器均从该共享模块调用数据拉取能力。
...
- 策略2沿用现有日线缓存和新鲜度策略。
- 缓存键、日线格式和过期判断不因策略类型改变。
- 流动性过滤使用完整拉取窗口。
- 策略2计算只使用策略窗口，不能因缓存中数据更多而扩大计算范围。
- 四数据源均失败时，只允许使用符合现有新鲜度要求的缓存；否则记录 `ALL_DATA_SOURCES_FAILED`。
...
- 数据源全部失败且无新鲜缓存时，返回 `ALL_DATA_SOURCES_FAILED`。
