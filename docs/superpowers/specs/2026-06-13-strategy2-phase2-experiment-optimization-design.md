...
- 用户已进一步授权：优化策略完成后，可以直接作为策略2正式版本启用，不需要用户再次确认具体参数。
...
- 当实验结果支持正式升级，并且已完成策略升级建议、测试验证和优化后策略文档时，可以直接将优化策略作为策略2正式版本启用。
- 启用正式版本不需要再次向用户确认具体参数数值。
- 启用后必须在最终交付中明确说明“已作为策略2正式版本启用”。
...
- 用不可比较任务结果作为正式调整依据。
- 在没有文档说明的情况下直接修改参数。
- 在测试失败、证据不足或回滚方案缺失时启用正式版本。

### 12.3 正式启用流程

当决定把优化策略升级为策略2正式版本时，必须按以下顺序执行：

1. 记录用于决策的可信基线任务 ID。
2. 记录用于决策的实验任务 ID。
3. 形成策略升级建议，说明为什么选择这些参数。
4. 修改策略2正式配置或正式扫描规则。
5. 更新前端配置页或展示文案。
6. 补充或更新测试。
7. 运行后端、前端和策略2相关回归测试。
8. 新增优化后的策略文档。
9. 在 `operations-log.md` 记录启用结果。
10. 最终交付中声明该策略已启用为策略2正式版本。
...
19. Phase 2 实验功能完成后，不要仅凭功能完成自动修改正式策略参数；必须先完成可信基线、可比较实验、策略升级建议和测试验证。
20. 用户已授权：当第12节触发条件满足且优化策略完成后，可以直接作为策略2正式版本启用，不需要再次确认具体参数。
21. 正式启用后必须交付优化后的策略文档，并写明最终参数名、参数值、证据、风险、回滚方案和是否已启用。
...
19. Phase 2完成后不要仅凭功能完成自动改正式策略参数；必须先完成可信基线、可比较实验、策略升级建议和测试验证。
20. 用户已授权：当实验结果支持正式升级，并且优化策略完成后，可以直接作为策略2正式版本启用，不需要再次确认具体参数。
...
13. 如果已经启用优化策略为正式版本，必须明确说明启用状态、启用版本和回滚方式。
