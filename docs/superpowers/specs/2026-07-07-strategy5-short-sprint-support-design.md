...
策略5需要至少 1000 个交易日用于长期历史过滤，同时至少 250 根 K 线用于 MA250。
...
| `minimum_trading_days` | 1000 | F1，上市/交易天数过滤 |
...
| 编号 | 条件 | 阈值 | 建议失败码 |
|---|---|---:|---|
| F1 | 交易天数 | `trading_days > 1000` | `TRADING_DAYS_LE_1000` |
...
1. **数据天数不足**：策略5要求 `>1000` 个交易日，当前部分扫描配置可能只取 350/500 根。实现时要为策略5单独提高 `kline_days`，但不能影响策略1-4。
