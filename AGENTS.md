...
策略6批量评分中的 `klineCollectionPatterns` 是可扩展的多K线诊断容器。`SLOW_ROUNDED_BASE_V2` 只在截至最新交易日的10-30日动态窗口中识别缓跌、减速、底部时间宽度、后段稳定和非V型结构；不得搜索已经结束的历史形态，不得以603976的固定天数、回撤或逐K路径作模板。集合形态只用于批量评分页展示和筛选，不得进入策略6资格、总分、排序、候选分类或生命周期。
