...
def scan_strategy4_all(config: dict, progress_callback=None, task_id: str | None = None, **kwargs) -> dict:
    ...
    if source is None:
        try:
            raw_topics = TopicSourceService().fetch_topics()
        ...
        leaders, candidates = _build_leaders_and_candidates_from_topics(topics, config, cfg)
...
def _build_leaders_and_candidates_from_topics(topics: list[dict], project_config: dict, strategy_config: dict) -> tuple[list[dict], list[dict]]:
