...
批量评分 `sellingExhaustion`（`strategy6/selling_exhaustion.py`）当前V3：仅展示与筛选，不参与正式策略评分、资格或排序。四因子原始分不变，保留V2的回调背景、离底和反弹排除及3日破位保护。新增反弹退出后至少两日持续承接且原始确认全部通过的回踩恢复路径；恢复必须补记反弹期间真实低点，再校验距离。五日收盘位置不足时，近3日均值>=0.55、比前2日提高>=0.10、最近两日各自>=普通阈值可补普通确认，但不加分或直接升级强/极致确认。评分等级与确认强度独立。配置为 `strategy6.selling_exhaustion`，缺省值在模块DEFAULTS；规则、真实历史匹配清单和局限见 `docs/reviews/2026-09-20-selling-exhaustion-v3.md`。
...
- **策略6批量评分：** `/strategy6/batch-evaluation` 通过 `/api/strategy6/batch-evaluate` 对用户输入的最多200只股票调用正式 `StrongVcpTailEngine.evaluate_at()`；输入支持普通代码列表及带“商品代码/股票代码/代码/Symbol/Ticker”表头的CSV，CSV只提取代码列。评分只读本地前复权日线和本地宽基指数，不拉行情、不创建扫描任务、不写候选或生命周期。页面同时展示原始尾部质量分和正式计入总分的尾部分：触发尾部硬条件时后者为0，前者仅用于比较接近合格程度，禁止据此绕过正式候选规则。点击结果行必须同时复制股票代码并展开详情；TradingView TXT只能导出当前筛选结果，使用带交易所前缀的英文逗号分隔格式。
- **策略6批量展示诊断：** 批量评分额外展示最新成交额是否为近5日最低、最新成交额是否不高于此前5个交易日平均值的60%、最新收盘是否低于MA5；此前5日成交额基准不得包含最新交易日。这些字段只作展示和筛选，不参与评分。`latest_bar_patterns` 可扩展输出最新交易日形态；锤子线固定要求实体占振幅10%-25%、下影不短于2倍实体、上影不超过实体10%，倒锤形使用镜像影线门槛，两者都要求振幅至少达到前收盘0.8%且不低于ATR14的50%。倒锤形必须结合此前5日位置区分倒锤子线、射击之星风险和位置不明确，不得使用未来K线确认。这些形态字段不得改变候选资格、总分、排序或生命周期。
- **策略6K线集合形态：** 批量评分使用通用 `klineCollectionPatterns` 输出多K线诊断。`SLOW_ROUNDED_BASE_V2` 动态比较所有截至最新交易日的10-30日窗口，识别缓跌、下跌减速、底部时间宽度、后段稳定和非V型结构。禁止固定603976的回撤、周期和逐K路径，也禁止返回已结束的历史窗口。该诊断只用于批量评分展示和筛选，不参与正式扫描、总分、排序、候选分类或生命周期。
