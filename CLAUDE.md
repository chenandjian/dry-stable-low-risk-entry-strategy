...
- **策略6批量展示诊断：** 批量评分额外展示最新成交额是否为近5日最低、最新收盘是否低于MA5，两者只读最近5根有效K线且不参与评分。`latest_bar_patterns` 可扩展输出最新交易日形态；锤子线固定要求实体占振幅10%-25%、下影不短于2倍实体、上影不超过实体10%，并要求振幅至少达到前收盘0.8%且不低于ATR14的50%。这些形态字段只用于诊断展示，不得改变候选资格、总分、排序或生命周期。
